...
const CartCard=(props) => {
    ...
    const [size, setSize]=useState(props.item.size.length>0? props.item.size[0]:'');
            ...
            <div className="product size">
                <Box sx={{ minWidth: 80 }}>
                    <FormControl fullWidth size="small">
                        ...
                        >
                            {Array.isArray(props.item.size)&&props.item.size.map((size) => <MenuItem value={size}>{size}</MenuItem>)}
                        </Select>
                    </FormControl>
                </Box>
            </div>
